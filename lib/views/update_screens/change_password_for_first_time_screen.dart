...
class ChangePasswordForFirstTimeScreen extends StatefulWidget {
  const ChangePasswordForFirstTimeScreen({super.key});

  @override
  State<ChangePasswordForFirstTimeScreen> createState() => _ChangePasswordForFirstTimeScreenState();
}

class _ChangePasswordForFirstTimeScreenState extends State<ChangePasswordForFirstTimeScreen> {
            ...
            child: Column(
              children: [
                ...
                Row(
                  ...
                  children: [
                    Text('حتى تتمكن من المتابعة يجب أن تقوم \n          بتغير كلمة المرور لأول مرة',style: TextStyle(fontSize: 15,fontWeight: FontWeight.w500,fontFamily: 'Roboto',color: Colors.black54),)
                  ],
                ),
                SizedBox(height: 50.h,),
                ...
                CustomButton(
                    ...
                    buttonText: 'تعديل', textStyle: TextStyle(fontSize: 16,color: Colors.white,fontWeight: FontWeight.w600,fontFamily: 'Roboto'), onPressed: ()async{
                      ...
                      if(profileController.passwordFormKey.currentState!.validate()){
                        final response = await profileController.changeUserPassword(oldPassword: profileController.oldPasswordController.text,
                            newPassword: profileController.newPasswordController.text,
                            newPasswordConfirmation: profileController.newPasswordConfirmationController.text);

                        response.fold((error){
                          EasyLoading.showError(error.apiErrorModel.message.toString(),dismissOnTap: false,duration: Duration(seconds: 3));
                        }, (successMessage){
                          EasyLoading.showSuccess(successMessage,duration: Duration(seconds: 3),dismissOnTap: true);
